...
Package still not published in CRAN repository, so it can be installed only from GitHub.
...
All standard LDA methods and parameters from `topimodels` package can be set with `method` and `control` arguments. Here use `seed` for repeatability of results.
